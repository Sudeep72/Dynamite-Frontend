...
import { Upload, FileText, Loader2, CheckCircle, XCircle, AlertCircle, X, FileImage } from 'lucide-react';
...
export const UploadView: React.FC = () => {
  ...
  const handleFileSelect = (selectedFiles: FileList | null) => {
    ...
    if (invalidFiles.length > 0) {
      ...
      setMessage(`${invalidFiles.length} file(s) rejected. Allowed formats: ${ALLOWED_EXTENSIONS.join(', ')}.`);
    }
  ...
  const getStatusIcon = (status: QueuedFile['uploadStatus']) => {
    switch (status) {
      ...
      case 'queued': return <FileText className="w-5 h-5 text-blue-600" />;
  ...
  const getStatusText = (status: QueuedFile['uploadStatus']) => {
    switch (status) {
      case 'ready': return <span className="text-green-600 text-base font-semibold ml-4 flex-shrink-0">Complete</span>;
      case 'queued': return <span className="text-gray-600 text-base font-semibold ml-4 flex-shrink-0">Queued</span>;
      case 'pending': return <span className="text-blue-600 text-base font-semibold ml-4 flex-shrink-0">Uploading...</span>;
      case 'failed': return <span className="text-red-600 text-base font-semibold ml-4 flex-shrink-0">Failed</span>;
      default: return <span className="text-gray-600 text-base font-semibold ml-4 flex-shrink-0">Processing</span>;
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto cursor-default">
      ...
      <div
        onDragEnter={() => { setIsDragging(true); clearMessages(); }}
        onDragLeave={() => setIsDragging(false)}
        onDragOver={(e) => e.preventDefault()}
        onDrop={handleDrop}
        className={`border-4 border-dashed rounded-xl p-16 text-center transition-colors ${ 
          isDragging ? 'border-violet-400 bg-violet-50' : 'border-blue-300 bg-white'
        }`}
      >
        <Upload className="w-14 h-14 text-blue-500 mx-auto mb-4" />
        <h3 className="text-gray-900 text-xl font-bold mb-3">Drag & Drop Images Here</h3>
              ...
              <Card key={file.id} className="p-5 flex items-center justify-between shadow-md border-blue-200">
                <CardContent className="p-0 flex items-center gap-4 min-w-0 flex-1">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-gray-900 font-medium text-base truncate">{file.name}</p>
          ...
          <div className="flex justify-end mt-6">
            ...
            >
              {uploading ? (
                ...
                <>
                  <Upload className="mr-2 h-5 w-5" /> Upload {files.length} File(s)
                </>
              )}
            </Button>
          </div>
      ...
      {files.length === 0 && !message && (
        <div className="p-6 bg-gray-50 rounded-xl mt-10 border border-gray-200">
          <h3 className="text-xl font-bold text-gray-900 mb-3">💡 Tips for Best Results</h3>
          <ul className="list-disc list-inside space-y-2 text-gray-600">
            <li><strong>Image Quality:</strong> Use clear, high-resolution images.</li>
            <li><strong>Variety:</strong> Include diverse images for better embeddings.</li>
            <li><strong>Format:</strong> PNG, JPG, JPEG, BMP, and WebP are supported.</li>
            <li><strong>Size:</strong> Recommended 500x500 pixels or larger.</li>
          </ul>
        </div>
      )}
    </div>
